...
class Exams extends Component {
    constructor(props) {
        ...
        this.PAGE_PATH = MyDataPath.EXAMS(this.props.academicYear);
    ...
    componentDidMount() {
        ...
        onValue(firebase.getListByRootName(authUser.uid, this.PAGE_PATH), (snapshot) => {
            ...
            if (subjectResult) {
                ...
                this.setState({examList: assList});
                return;
            }
            // get subject list
    ...
    handleShow() {
        ...
        if (subjectList.length == 0) {
            alert("No subjects currently, you need to add subjects before adding exams!");
            return;
        }
    ...
    render() {
        const {show, showEdit, editCompleteFlag, tabValue, datePickValue, subjectList, examList} = this.state;
            ...
            <div className='subjects'>
                ...
                <Box sx={{width: '100%'}}>
                    ...
                    <Box sx={{height: 480, maxHeight: 480, overflow: 'auto', borderBottom: 1, borderColor: 'divider'}}>
                        {examList.sort(this.sortByTabValue()).filter(this.filterByTabValue()).length == 0 && (
                        ...
                        {examList.sort(this.sortByTabValue()).filter(this.filterByTabValue()).length > 0 && (
                            examList.sort(this.sortByTabValue()).filter(this.filterByTabValue()).map((item) => (
                    ...
                    <Modal.Body>
                        <form onSubmit={handleSubmit((data) => {
                            ...
                            {errors.title?.type === 'required' && (
                                <div className='subjectsForm'>
                                    <span className="warning-color">Exam Title is required</span>
                                </div>
                            )}
                            <div className='subjectsForm'>
...
export default compose(withAcademicYear, withFormHOC, withFirebase, withAuthUser, withAuthorization(condition))(Exams);